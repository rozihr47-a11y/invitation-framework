(function () {
  ...
  // Optimized pixel ratio (prevents GPU overkill)
  ...
  const loadingManager = new THREE.LoadingManager(() => {
    ...
    // Assign textures ONLY after all loaded
    material.uniforms.uTexture1.value = texture1;
    material.uniforms.uTexture2.value = texture1; // TEMP: force same texture
    material.uniforms.uDisp.value = disp;
    
    material.uniforms.dispFactor.value = 0;
    
    // First clean render (GPU warm-up)
    renderer.render(scene, camera);
    
    // Now switch to real texture2 AFTER first frame
