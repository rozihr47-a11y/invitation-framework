...
// console.log("Is html?", scrollLockTarget === document.documentElement);
 ...
 gsap.to(scrollLockTarget, {
    ...
    scrollTrigger: {
      ...
      start: "15% 40%",
      // markers: true,
      toggleActions: "play none none none" // Controls enter and leave behavior
    }
});

  const Tl_frameLeave = gsap.timeline ({paused: true});

  
          // .set(scrollLockTarget, { overflow: "hidden", duration: 0})
    ...
    const Tl_quote= gsap.timeline({
      ...
      // play when scroll reaches end
      if (self.progress > 0.92 && !framePlayed) {
          framePlayed = true;
          // Use a small timeout to prevent the animation play() call from blocking the main thread
          // right after a scroll event, which can cause jank.
          setTimeout(() => {
      ...
      if (self.progress < 0.92 && framePlayed) {
          ...
          // raysTL.timeScale(2).pause();          // or reverse()
